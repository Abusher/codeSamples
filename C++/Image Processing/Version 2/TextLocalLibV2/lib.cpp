...
void	Voronoi::InsertParabola(Point2d * p)
{
	...
	Point2d * start = new Point2d(p->x, GetY(par->site, p->x));
...
void	Voronoi::RemoveParabola(VEvent * e)
{
	...
	Point2d * p = new Point2d(e->point->x, GetY(p1->site, e->point->x));
	points.push_back(p);
...
void	Voronoi::FinishEdge(VParabola * n)
{
	...
	Point2d * end = new Point2d(mx, mx * n->edge->f + n->edge->g);
	n->edge->end = end;
...
double	Voronoi::GetY(Point2d * p, double x) 
{
	double dp = 2 * (p->y - ly);
	double a1 = 1 / dp;
	double b1 = -2 * p->x / dp;
...
void	Voronoi::CheckCircle(VParabola * b)
{
	...
	VEvent * e = new VEvent(new Point2d(s->x, s->y - d), false);
...
Point2d * Voronoi::GetEdgeIntersection(VEdge * a, VEdge * b)
{
